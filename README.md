...
- 安装方法一<PIP源直接安装>：
  ...
  ```
    > 该方法会默认下载安装`paddlepaddle v1.7.2 cpu版本`，若提示PaddlePaddle无法安装按下面操作：
    > - 可以在[该地址](https://pypi.org/project/paddlepaddle/1.7.2/#files)，下载PaddlePaddle后手动安装whl包
    > - 可以先pip安装PaddlePaddle，`python -m pip install paddlepaddle==1.7.2 -i https://mirror.baidu.com/pypi/simple`
