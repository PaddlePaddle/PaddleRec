...
## 数据准备
本模型使用论文中的ag_news数据集，在模型目录的data目录下为您准备了快速运行的示例数据，若需要使用全量数据可以参考下方[效果复现]部分，数据的格式如下：  
```
...
```
# 进入模型目录
cd models/rank/tagspace # 在任意目录均可运行
# 动态图训练
...
2. 进入tools/datasets/ag_news目录下，执行该脚本，会从国内源的服务器上下载我们预处理完成的ag_news全量数据集，并解压到指定文件夹。
``` 
cd ../../../tools/datasets/ag_news
sh run.sh
```
3. 切回模型目录,执行命令运行全量数据
```
