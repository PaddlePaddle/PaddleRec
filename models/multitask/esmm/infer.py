...
def main(args):
    ...
    for epoch_id in range(start_epoch + 1, end_epoch):
        ...
        load_model(model_path, esmm_model)
        for batch_id, batch in enumerate(test_dataloader()):
            ...
            if batch_id % print_interval == 1:
                logger.info(
                    "infer epoch: {}, batch_id: {}, ctr_auc: {:.6f}, ctcvr_auc: {:.5f}, speed: {:.2f} ins/s".
                    format(epoch_id, batch_id,
                           ctr_auc_metric.accumulate(),
                           ctcvr_auc_metric.accumulate(), print_interval *
                           batch_size / (time.time() - interval_begin)))
                interval_begin = time.time()

        logger.info(
            "infer epoch: {} done, ctr_auc: {:.6f}, ctcvr_auc: {:.5f}, epoch time{:.2f} s".
            format(epoch_id,
                   ctr_auc_metric.accumulate(),
