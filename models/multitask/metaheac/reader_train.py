    ...
    def __iter__(self):
        ...
        for file in self.file_list:
            with open(file, "rb") as rf:
                ...
                n_batch = int(np.ceil(
                    n_samples / self.batchsize))  #总量除以batchsize * task_count
                ...
                for i_batch in range(n_batch):
                    ...
                    list_sup_x, list_sup_y, list_qry_x, list_qry_y = list(
                    ), list(), list(), list()
                    ...
                    for aid in batch_aid_set:
                        ...
                        for k in range(len(self.dynamic_context_col)):
                            dy_np = np.array(batch_sup[
                                self.dynamic_context_col[k]])[:]
                        ...
                        batch_sup_x.append(
                            ...
                            [:])  #[batchsize,7]
                        batch_sup_x.append(
                            np.array(batch_sup[self.ad_col])
                            [:])  #[batchsize,6]
                        ...
                        batch_qry_x.append(
                            ...
                            [:])  #[batchsize,7]
                        batch_qry_x.append(
                            np.array(batch_qry[self.ad_col])
                            [:])  #[batchsize,6]
                        ...
                        list_sup_y.append(
                            batch_sup_y)  # shape = [5,batchsize,1]
                        list_qry_x.append(
                            batch_qry_x)  # shape = [5,batchsize,7+50+7+6]
                        list_qry_y.append(
                            batch_qry_y)  # shape = [5,batchsize,1]
