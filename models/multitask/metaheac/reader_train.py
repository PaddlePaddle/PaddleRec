...
class RecDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        with open(file, "rb") as rf:
            ...
            n_batch = int(np.ceil(n_samples /
                                  self.batchsize))  #总量除以batchsize * task_count
            list_prob = []
            for aid in aid_set:
                list_prob.append(data_train_stage1[data_train_stage1.aid ==
                                                   aid].shape[0])
            ...
            for i_batch in range(n_batch):
                batch_aid_set = np.random.choice(
                    aid_set, size=self.task_count, replace=True, p=list_prob)

                list_sup_x, list_sup_y, list_qry_x, list_qry_y = list(), list(
                ), list(), list()

                for aid in batch_aid_set:
                    batch_sup = data_train[data_train.aid ==
                                           aid].sample(self.batchsize)
                    batch_qry = data_train[data_train.aid ==
                                           aid].sample(self.batchsize)

                    batch_sup_x = []
                    batch_sup_x.append(
                        np.array(batch_sup[self.static_context_col])
                        [:])  #[batchsize,7]

                    # sup中dynamic部分
                    temp_list = list()
                    for k in range(len(self.dynamic_context_col)):
                        dy_np = np.array(batch_sup[self.dynamic_context_col[
                            k]])[:]
                    ...
                    batch_sup_x.append(
                        np.array(batch_sup[self.col_length_name])
                        [:])  #[batchsize,7]
                    batch_sup_x.append(
                        np.array(batch_sup[self.ad_col])[:])  #[batchsize,6]

                    batch_sup_y = np.array(batch_sup[self.label_col]
                                           .values)[:]  #[batchsize,1]

                    batch_qry_x = []
                    batch_qry_x.append(
                        np.array(batch_qry[self.static_context_col])
                        [:])  #[batchsize,7]

                    # qry中dynamic部分
                    temp_list = list()
                    for k in range(len(self.dynamic_context_col)):
                        dy_np = np.array(batch_qry[self.dynamic_context_col[
                            k]])[:]
                        dy_np = np.vstack(dy_np)
                        temp_list.append(dy_np)
                    temp_np = np.concatenate(temp_list, axis=1)
                    ...
                    batch_qry_x.append(
                        np.array(batch_qry[self.col_length_name])
                        [:])  #[batchsize,7]
                    batch_qry_x.append(
                        np.array(batch_qry[self.ad_col])[:])  #[batchsize,6]

                    batch_qry_y = np.array(batch_qry[self.label_col]
                                           .values)[:]  #[batchsize,1]

                    list_sup_x.append(
                        batch_sup_x)  # shape = [5,batchsize,7+50+7+6]
                    list_sup_y.append(batch_sup_y)  # shape = [5,batchsize,1]
                    list_qry_x.append(
                        batch_qry_x)  # shape = [5,batchsize,7+50+7+6]
