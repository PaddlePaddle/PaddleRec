...
def main(args):
    ...
    for epoch_id in range(start_epoch, end_epoch):
        ...
        click_labels, click_preds, conversation_labels, conversation_preds = [], [], [], []
        for batch_id, batch in enumerate(test_dataloader()):
            ...
            click, click_pred, conversion, conversion_pred = dy_model_class.forward(
                dy_model, batch, config
            )
            click_labels.append(click.cpu().numpy())
            click_preds.append(click_pred.cpu().numpy())
            conversation_labels.append(conversion.cpu().numpy())
            conversation_preds.append(conversion_pred.cpu().numpy())
        ...
        click_labels = np.concatenate(click_labels, axis=0)
        click_preds = np.concatenate(click_preds, axis=0)
        conversation_labels = np.concatenate(conversation_labels, axis=0)
        conversation_preds = np.concatenate(conversation_preds, axis=0)
        click_auc = roc_auc_score(click_labels, click_preds)
        conversation_auc = roc_auc_score(conversation_labels, conversation_preds)
        logger.info('click auc score: {:.6f}'.format(click_auc))
        logger.info('purchase auc score: {:.6f}'.format(conversation_auc))
        epoch_begin = time.time()
