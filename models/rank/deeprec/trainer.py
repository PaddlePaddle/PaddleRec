...
def main(args):
    ...
    optimizer, scheduler = dy_model_class.create_optimizer(dy_model, config)
    ...
    for epoch_id in range(last_epoch_id + 1, epochs):
        ...
        for batch_id, batch in enumerate(train_dataloader()):
            ...
            outputs, loss, metric_list, tensor_print_dict = \
                dy_model_class.train_forward(
            ...
            aug_step = config.get("runner.aug_step", 1)
            if aug_step > 0:
                # Magic data augmentation trick happen here
                for t in range(aug_step):
                    optimizer.clear_grad()
                    reoutputs, loss, metric_list, aug_tensor_print_dict = \
                        dy_model_class.train_forward(
                            dy_model, metric_list, outputs, config
                            )
                    loss.backward()
                    optimizer.step()
                    optimizer.clear_grad()
