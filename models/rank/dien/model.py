...
class Model(ModelBase):
    ...
    def net(self, inputs, is_infer=False):
        ...
        #limit = fluid.layers.assign(np.array([1], dtype='float32'))
        ...
        #self._metrics["GRADIENT"] = item_emb_copy  #fluid.global_scope().find_var("item_emb").get_tensor()

        #self._metrics["EMBD"] = fluid.layers.reduce_mean(hist_seq_concat)


'''        self._metrics["GRU1"] = fluid.layers.reduce_mean(gru_h1)
        self._metrics["ATTENTION"] = fluid.layers.reduce_mean(attention_rnn_res)
        self._metrics["LOGITS"]=fluid.layers.reduce_mean(logit)
        self._metrics["PREDICT2D"]=fluid.layers.reduce_mean(predict_2d)


        self._metrics["EMBEDDINS"] = item_emb_attr
        self._metrics["GRUinShape"] = fluid.layers.shape(gru_input_hist_item_emb)
        self._metrics["GRU1Shape"] = fluid.layers.shape(gru_h1)
        self._metrics["GRU2Shape"] = fluid.layers.shape(gru_h2)
        self._metrics["GRUPadShape"] = fluid.layers.shape(gru_out_pad)

       self._metrics["INPUTS"] = fluid.layers.reduce_mean(hist_item_seq)
        self._metrics["EMBD"] = fluid.layers.reduce_mean(hist_seq_concat)
        self._metrics["GRU1"] = fluid.layers.reduce_mean(gru_h1)
        self._metrics["ATTENTION"] = fluid.layers.reduce_mean(attention_rnn_res)
        self._metrics["LOGITS"]=fluid.layers.reduce_mean(logit)
