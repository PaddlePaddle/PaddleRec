...
class DINLayer(nn.Layer):
    def __init__(self, item_emb_size, cat_emb_size,act,
        ...
        sizes = [(self.item_emb_size + self.cat_emb_size)*4] + [80] + [40] + [1]
        acts = ["relu" for _ in range(len(sizes)-2)] + [None]

        for i in range(len(sizes)-1):
            linear=paddle.nn.Linear(in_features=sizes[i],out_features=sizes[i + 1],weight_attr=paddle.ParamAttr(
        ...
        linearCon = paddle.nn.Linear(in_features=self.firInDim, out_features=self.firOutDim, weight_attr=paddle.ParamAttr(
            initializer=paddle.nn.initializer.Normal(
        ...
        for i in range(len(conSizes) - 1):
            linear = paddle.nn.Linear(in_features=conSizes[i], out_features=conSizes[i + 1], weight_attr=paddle.ParamAttr(
                initializer=paddle.nn.initializer.Normal(
    ...
    def forward(self, hist_item_seq, hist_cat_seq, target_item, target_cat, label, mask, target_item_seq, target_cat_seq):
        ...
        concat = paddle.concat(
            [hist_seq_concat, target_seq_concat, hist_seq_concat - target_seq_concat, hist_seq_concat * target_seq_concat],
            axis=2)
        ...
        #[b, 1, row]
        output = paddle.matmul(weight, hist_seq_concat)   # X's shape: [2, 1, 512],Y's shape: [256, 80]
        #[b,row]
...
class StaticDINLayer(nn.Layer):
    def __init__(self, item_emb_size, cat_emb_size,act,
        ...
        linearCon = paddle.nn.Linear(in_features=self.firInDim, out_features=self.firOutDim,
                                     weight_attr=paddle.ParamAttr(
                                         initializer=paddle.nn.initializer.Normal(
                                             std=1.0 / math.sqrt(self.firInDim))), )
        self.add_sublayer('linearCon', linearCon)
        ...
        for i in range(len(conSizes) - 1):
            linear = paddle.nn.Linear(in_features=conSizes[i], out_features=conSizes[i + 1],
                                      weight_attr=paddle.ParamAttr(
                                          initializer=paddle.nn.initializer.Normal(
                                              std=1.0 / math.sqrt(conSizes[i]))), )
            self.add_sublayer('linear_%d' % i, linear)
    ...
    def forward(self, hist_item_seq, hist_cat_seq, target_item, target_cat, label, mask, target_item_seq,
                target_cat_seq):
        hist_item_emb = self.hist_item_emb_attr(hist_item_seq)
        hist_cat_emb = self.hist_cat_emb_attr(hist_cat_seq)
        target_item_emb = self.target_item_emb_attr(target_item)
        ...
        concat = paddle.concat(
            [hist_seq_concat, target_seq_concat, paddle.elementwise_sub(hist_seq_concat, target_seq_concat),
        ...
        paddle.static.Print(logit, first_n=- 1, message=None, summarize=20, print_tensor_name=True,
                            print_tensor_type=True, print_tensor_shape=True, print_tensor_lod=True, print_phase='both')
