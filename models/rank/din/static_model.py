...
from net import DINLayer, StaticDINLayer
...
class StaticModel():
    ...
    def _init_hyper_parameters(self):
        ...
        self.item_emb_size = self.config.get(
            "hyper_parameters.item_emb_size", 64)
        self.cat_emb_size = self.config.get(
            "hyper_parameters.cat_emb_size", 64)
        self.act = self.config.get(
            "hyper_parameters.act", "sigmoid")
        self.is_sparse = self.config.get("hyper_parameters.is_sparse", False)
        self.use_DataLoader = self.config.get("hyper_parameters.use_DataLoader", False)
        self.item_count = self.config.get(
    ...
    def create_feeds(self, is_infer=False):
        ...
        target_cat = paddle.static.data(name="target_cat", shape=[None], dtype="int64")
    ...
    def net(self, inputs, is_infer=False):
        ...
        # StaticDINLayer
        din_model = DINLayer(self.item_emb_size, self.cat_emb_size, self.act, self.is_sparse, self.use_DataLoader, self.item_count, self.cat_count)

        raw_predict= din_model(self.hist_item_seq, self.hist_cat_seq, self.target_item, self.target_cat, self.label, self.mask, self.target_item_seq, self.target_cat_seq)
        ...
        loss = paddle.nn.functional.cross_entropy(
            input=raw_predict, label=paddle.cast(self.label, "float32"), soft_label=True)
