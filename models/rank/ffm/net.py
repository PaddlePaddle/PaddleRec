...
class FFM(nn.Layer):
        ...
        self.embedding_one = paddle.nn.Embedding(
            ...
            weight_attr=paddle.ParamAttr(
                initializer=paddle.nn.initializer.TruncatedNormal(
                    mean=0.0,
                    std=self.init_value_ / math.sqrt(
                        float(self.sparse_feature_dim *
    ...
    def forward(self, sparse_inputs, dense_inputs):
        ...
        field_aware_interaction_concat = paddle.concat(
            field_aware_interaction_list, axis=1)
        y_field_aware_second_order = paddle.sum(field_aware_interaction_concat,
                                                1)
