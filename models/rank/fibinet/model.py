...
class Model(ModelBase):
    ...
    def _SENETLayer(self, inputs, filed_size, reduction_ratio = 3):
        ...
        A_1 = fluid.layers.fc(input=Z,
                            size=reduction_size, 
                            param_attr=fluid.initializer.Xavier(uniform=False),
                            act='relu',
                            name='W_1')

        A_2 = fluid.layers.fc(input=A_1,
                            size=filed_size, 
                            param_attr=fluid.initializer.Xavier(uniform=False),
                            act='relu',
                            name='W_2')
    ...
    def _BilinearInteraction(self, inputs, filed_size, embedding_size, bilinear_type="interaction"):
        if bilinear_type == "all":
            p = [fluid.layers.elementwise_mul(fluid.layers.fc(input=v_i,
                            size=embedding_size, 
                            param_attr=fluid.initializer.Xavier(uniform=False),
                            act=None,
                            name=None), fluid.layers.squeeze(input=v_j, axes=[1]))
    ...
    def _DNNLayer(self, inputs, dropout_rate=0.5):
        ...
        for i, hidden_unit in enumerate([400, 400, 400]):
            fc_out = fluid.layers.fc(input=deep_input,
                                    size=hidden_unit, 
                                    param_attr=fluid.initializer.Xavier(uniform=False),
                                    act='relu',
                                    name='d_' + str(i))
    ...
    def net(self, input, is_infer=False): 
        ...
        for data in self.sparse_inputs:
            feat_emb = fluid.embedding(input=data,
                                ...
                                param_attr=fluid.ParamAttr(name='dis_emb',
                                                            learning_rate=5,
                                                            initializer=fluid.initializer.Xavier(fan_in=self.sparse_feature_dim,fan_out=self.sparse_feature_dim)
                                                            ),
                                is_sparse=True)
            emb.append(feat_emb)
        ...
        senet_output = self._SENETLayer(concat_emb, filed_size, reduction_ratio)
        senet_bilinear_out = self._BilinearInteraction(senet_output, filed_size, self.sparse_feature_dim, bilinear_type)

        concat_emb = fluid.layers.split(concat_emb, num_or_sections=filed_size, dim=1)
        bilinear_out = self._BilinearInteraction(concat_emb, filed_size, self.sparse_feature_dim, bilinear_type)
        ...
        y_pred = fluid.layers.fc(input=dnn_output,
                                size=1, 
                                param_attr=fluid.initializer.Xavier(uniform=False),
                                act='sigmoid',
                                name='logit')
