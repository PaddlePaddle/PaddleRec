...
class GateAttention(paddle.nn.Layer):
    def __init__(self, hidden_size):
        super().__init__()
        self.w1 = nn.Linear(hidden_size, hidden_size, weight_attr=ParamAttr(initializer=initializer.Normal(std=0.1)),
                            bias_attr=False)
        self.w2 = nn.Linear(hidden_size, hidden_size, weight_attr=ParamAttr(initializer=initializer.Normal(std=0.1)), )
...
class Attention(paddle.nn.Layer):
    def __init__(self, hidden_size1, hidden_size2, dropout_rate):
        super().__init__()
        self.w_omega = nn.Linear(hidden_size2, 1, weight_attr=ParamAttr(initializer=initializer.Normal(std=0.1)))
        self.w = nn.Linear(hidden_size1, hidden_size2, weight_attr=ParamAttr(initializer=initializer.Normal(std=0.1)),
                           bias_attr=False)
        self.u_omega = paddle.create_parameter([1], 'float32', default_initializer=initializer.Normal(std=0.1))
...
class IntraLayer(nn.Layer):
    def __init__(self, hidden_size, f_max_len):
        ...
        self.w_k = nn.Linear(hidden_size, hidden_size, weight_attr=ParamAttr(initializer=initializer.XavierNormal()),
                             bias_attr=False)
        self.w_i = nn.Linear(hidden_size, hidden_size, weight_attr=ParamAttr(initializer=initializer.XavierNormal()),
                             bias_attr=False)
        self.u_omega = paddle.create_parameter([1], 'float32', default_initializer=initializer.Normal(std=0.1))
        self.w_omega = nn.Linear(2 * hidden_size, 1, weight_attr=ParamAttr(initializer=initializer.Normal(std=0.1)), )

    def forward(self, item_emb, friend_emb, masks):
        f_k_emb = self.w_k(friend_emb)
        _item = self.w_i(item_emb).unsqueeze(1)
        inputs = paddle.concat([paddle.tile(_item, [1, self.f_max_len, 1]), f_k_emb], -1)
        v = F.tanh(self.w_omega(inputs))
...
class IPRECLayer(paddle.nn.Layer):
    def __init__(self,
        ...
        self.user_emb = nn.Embedding(self.user_num + 1, hidden_units, padding_idx=0,
                                     weight_attr=ParamAttr(initializer=initializer.XavierUniform()))
        self.biz_emb = nn.Embedding(self.biz_num + 1, hidden_units, padding_idx=0,
                                    weight_attr=ParamAttr(initializer=initializer.XavierUniform()))

        self.friend_attn = Attention(2 * hidden_units, hidden_units, dropout_rate)
        self.item_attn = Attention(2 * hidden_units, hidden_units, dropout_rate)
        self.biz_attn = Attention(2 * hidden_units, hidden_units, dropout_rate)
        self.type_attn = Attention(2 * hidden_units, hidden_units, dropout_rate)
        self.pack_attn = Attention(2 * hidden_units, hidden_units, dropout_rate)
        self.w_self = nn.Linear(2 * hidden_units, hidden_units,
                                weight_attr=ParamAttr(initializer=initializer.XavierUniform()), bias_attr=False)

        self.intra_layers = nn.LayerList([IntraLayer(hidden_units, f_max_len) for _ in range(self.K)])
        self.dropout = nn.Dropout(dropout_rate)

        self.w_a = nn.Linear(2 * hidden_units, hidden_units,
                             weight_attr=ParamAttr(initializer=initializer.XavierUniform()), bias_attr=False)
        self.u_omega = paddle.create_parameter([1], 'float32', default_initializer=initializer.Normal(std=0.1))
        self.w_omega = nn.Linear(hidden_units, 1, weight_attr=ParamAttr(initializer=initializer.Normal(std=0.1)), )
    ...
    def forward(self, user, item, biz, friends, user_items, user_bizs, user_friends, user_packages,
                pack_neighbors_b, pack_neighbors_f):
        ...
        up_mask = user_packages.sum(-1).abs().astype('float32').sign().unsqueeze(-1)
        pb_mask = pack_neighbors_b.sum(-1).abs().astype('float32').sign().unsqueeze(-1)
        pf_mask = pack_neighbors_f.sum(-1).abs().astype('float32').sign().unsqueeze(-1)
        up_items, up_bizs, up_friends = paddle.split(user_packages, [1, 1, self.f_max_len],
                                                     axis=-1)  # B * u_max_p * 1, B * u_max_p * 1, B * u_max_p * max_f
        pb_items, pb_bizs, pb_friends = paddle.split(pack_neighbors_b, [1, 1, self.f_max_len],
                                                     axis=-1)  # B * p_max_nei(biz) * 1
        pf_items, pf_bizs, pf_friends = paddle.split(pack_neighbors_f, [1, 1, self.f_max_len],
                                                     axis=-1)  # B * p_max_nei(fri) * 1
        _items = paddle.concat([paddle.reshape(item, [-1, 1, 1]), up_items, pb_items, pf_items], axis=1)
        _bizs = paddle.concat([paddle.reshape(biz, [-1, 1, 1]), up_bizs, pb_bizs, pf_bizs], axis=1)
        _friends = paddle.concat([friends.unsqueeze(1), up_friends, pb_friends, pf_friends], axis=1)
        user_emb, a2, a3, ta = self.dual_aggregate(user_emb, user_items, user_bizs, user_friends)

        intra_packages, att = self.intra(user_emb, paddle.reshape(_friends, [-1, self.f_max_len]),
                                         paddle.reshape(_items, [-1]),
                                         paddle.reshape(_bizs, [-1]))
        intra_packages = paddle.reshape(intra_packages, [batch_size, -1, self.hidden_units])
        att = paddle.reshape(att, [batch_size, -1, 7])
        tar_pack, u_packs, pb_packs, pf_packs = paddle.split(
            intra_packages, [1, self.u_max_pack, self.pack_max_nei_b, self.pack_max_nei_f], axis=1)
        tar_att, u_att, pb_att, pf_att = paddle.split(
            att, [1, self.u_max_pack, self.pack_max_nei_b, self.pack_max_nei_f], axis=1)
        ...
        pb_packs = paddle.reshape(pb_packs, [batch_size, -1, self.hidden_units])
        pf_packs = paddle.reshape(pf_packs, [batch_size, -1, self.hidden_units])
        ...
        agg_out = self.dnn(paddle.concat([user_emb, pack_emb, user_emb * pack_emb], 1))
    ...
    def intra(self, user_emb, friends, item, biz):
        _user_emb = paddle.reshape(
            paddle.tile(user_emb.unsqueeze(1), [1, 1 + self.u_max_pack + self.pack_max_nei_b + self.pack_max_nei_f, 1]),
            [-1, self.hidden_units, 1]
        )  # BN*D*1
        ...
        t_user = paddle.reshape(paddle.tile(user_emb.unsqueeze(1),
                                            [1, 1 + self.u_max_pack + self.pack_max_nei_b + self.pack_max_nei_f, 1]),
                                [-1, 1, self.hidden_units])
        inputs = paddle.concat([paddle.tile(t_user, [1, self.K, 1]), f_K_emb], -1)
        ...
        pack = paddle.stack([f_emb, item_emb, biz_emb, f_emb * item_emb, f_emb * biz_emb, item_emb * biz_emb,
                             f_emb * item_emb * biz_emb], 1)
        masks = pack.sum(-1).abs().astype('float32').sign()
        _user_emb = paddle.tile(paddle.transpose(_user_emb, perm=[0, 2, 1]), [1, 7, 1])
    ...
    def dual_aggregate(self, user_emb, items, bizs, friends):
        ...
        user_emb = F.relu(self.w_self(paddle.concat([_user_emb, user_emb], -1)))
