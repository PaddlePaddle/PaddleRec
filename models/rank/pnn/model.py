...
class Model(ModelBase):
    ...
    def net(self, inputs, is_infer=False):
        ...
        flaten_feat_embedding = paddle.fluid.layers.nn.reshape(
            x=feat_embeddings, shape=[-1, linear_input_size])

        w_z_linear_weights = paddle.create_parameter(
            shape=[linear_input_size, self.deep_input_size], dtype="float32")
        linear_signal = paddle.fluid.layers.matmul(
        ...
        if self.use_inner_product:
            w_p_quardatic_weights = paddle.create_parameter(
                shape=[self.deep_input_size, self.num_field], dtype="float32")
            for i in range(self.deep_input_size):
                transpose_embedding = paddle.transpose(
                    feat_embeddings, perm=[0, 2, 1])
                theta = paddle.multiply(
                    x=transpose_embedding, y=w_p_quardatic_weights[i], axis=-1)
                quadratic_output.append(
                    paddle.norm(
                        paddle.sum(x=theta, axis=1), p=2, axis=1,
                        keepdim=True))
        else:
            w_p_quardatic_weights_outer = paddle.create_parameter(
                shape=[
                    self.deep_input_size, self.sparse_feature_dim,
                    self.sparse_feature_dim
                ],
                dtype="float32")
            embedding_sum = paddle.sum(x=feat_embeddings, axis=1)
            p = paddle.fluid.layers.matmul(
            ...
            for i in range(self.deep_input_size):
                theta = paddle.multiply(
                    x=p, y=w_p_quardatic_weights_outer[i, :, :], axis=-1)
                quadratic_output.append(
                    paddle.fluid.layers.nn.reshape(
                        paddle.sum(x=theta, axis=[1, 2]), shape=[-1, 1]))

        quadratic_signal = paddle.concat(x=quadratic_output, axis=1)

        y_dnn = linear_signal + quadratic_signal

        y_dnn = paddle.fluid.layers.nn.relu6(y_dnn, threshold=10000000.0)
