...
class Reader(fleet.MultiSlotStringDataGenerator):
        ...
        for idx in continuous_range_:
            if features[idx] == "":
                ...
                dense_feature.append(
                    ...
                    cont_diff_[idx - 1])
        dense_feature = [str(x) for x in dense_feature]
        for idx in categorical_range_:
            sparse_feature.append(
                [str(hash(str(idx) + features[idx]) % hash_dim_)])
        label = [features[0]]
        res = [lineid] + [label] + sparse_feature + [dense_feature]
        return res
    ...
    def generate_sample(self, line):
        ...
        def reader():
            ...
            feature_name = ["insid", "label"]
            for idx in categorical_range_:
                feature_name.append("C" + str(idx - 13))
            feature_name.append("dense_input")
