...
class CIN(paddle.nn.Layer):
    ...
    def forward(self, feat_embeddings):
        ...
        for s in self.layer_sizes_cin:
            ...
            w_var = paddle.randn(
                (s, last_s * self.num_field, 1, 1), dtype='float32')
            X_k_1 = paddle.nn.functional.conv2d(Z_k_1, w_var)
...
class StaticxDeepFMLayer(paddle.nn.Layer):
    def __init__(self, sparse_feature_number, sparse_feature_dim,
                 dense_feature_dim, sparse_num_field, layer_sizes_cin,
                 layer_sizes_dnn):
        super(StaticxDeepFMLayer, self).__init__()
        self.sparse_feature_number = sparse_feature_number
        self.sparse_feature_dim = sparse_feature_dim
        self.dense_feature_dim = dense_feature_dim
        self.sparse_num_field = sparse_num_field
        ...
        self.linear = StaticLinear(sparse_feature_number,
                                   dense_feature_dim + sparse_num_field,
                                   sparse_feature_dim, dense_feature_dim)
        self.cin = StaticCIN(sparse_feature_dim,
                             dense_feature_dim + sparse_num_field,
                             layer_sizes_cin)

        self.dnn = StaticDNN(sparse_feature_dim,
                             dense_feature_dim + sparse_num_field,
                             layer_sizes_dnn)
...
class StaticLinear(paddle.nn.Layer):
    def __init__(self, sparse_feature_number, num_field, sparse_feature_dim,
                 dense_feature_dim):
    ...
    def forward(self, sparse_inputs, dense_inputs):
        sparse_inputs_concat = paddle.concat(sparse_inputs, axis=1)
        sparse_emb_one = paddle.static.nn.embedding(
            input=sparse_inputs_concat, size=[self.sparse_feature_number, 1])

        dense_emb_one = paddle.multiply(dense_inputs, self.dense_w_one)
        dense_emb_one = paddle.unsqueeze(dense_emb_one, axis=2)

        sparse_embeddings = paddle.static.nn.embedding(
            input=sparse_inputs_concat,
...
class StaticCIN(paddle.nn.Layer):
    def __init__(self, sparse_feature_dim, num_field, layer_sizes_cin):
        super(StaticCIN, self).__init__()
        self.sparse_feature_dim = sparse_feature_dim
        self.num_field = num_field
        self.layer_sizes_cin = layer_sizes_cin
    ...
    def forward(self, feat_embeddings):
        ...
        for s in self.layer_sizes_cin:
            ...
            X_k_1 = paddle.static.nn.conv2d(
                input=Z_k_1, num_filters=s, filter_size=(1, 1))
            X_k_1 = paddle.reshape(
                x=X_k_1,
                shape=[-1, s,
                       self.sparse_feature_dim])  # None, s, embedding_size
        ...
        y_cin = paddle.sum(x=y_cin, axis=-1)  # None, (num_field++)
        tmp_sum = sum(self.layer_sizes_cin)
        cin_linear = paddle.static.nn.fc(
            x=y_cin,
            size=1,
            weight_attr=paddle.ParamAttr(
...
class StaticDNN(paddle.nn.Layer):
    def __init__(self, sparse_feature_dim, num_field, layer_sizes_dnn):
        super(StaticDNN, self).__init__()
        self.sparse_feature_dim = sparse_feature_dim
        self.num_field = num_field
        self.layer_sizes_dnn = layer_sizes_dnn
