...
class xDeepFMLayer(paddle.nn.Layer):
    def __init__(self, sparse_feature_number, sparse_feature_dim,
        ...
        self.linear = Linear(sparse_feature_number, dense_feature_dim + sparse_num_field,
                       sparse_feature_dim, dense_feature_dim)
        self.cin = CIN(sparse_feature_dim, dense_feature_dim + sparse_num_field,  
                       layer_sizes_cin)

        self.dnn = DNN(sparse_feature_dim, dense_feature_dim + sparse_num_field,
                       layer_sizes_dnn)
...
class Linear(paddle.nn.Layer):
    ...
    def forward(self, sparse_inputs, dense_inputs):
        ...
        y_linear = paddle.sum(sparse_emb_one, 1) + paddle.sum(
            dense_emb_one, 1) + b_linear

        return y_linear, feat_embeddings

class CIN(paddle.nn.Layer):
    def __init__(self, sparse_feature_dim,
            num_field, layer_sizes_cin):
    ...
    def forward(self, feat_embeddings):
        ...
        for s in self.layer_sizes_cin:
            ...
            Z_k_1 = paddle.reshape(x=Z_k_1, shape=[
            ...
            Z_k_1 = paddle.reshape(
                x=Z_k_1,
                shape=[-1, last_s * self.num_field, 1, self.sparse_feature_dim]
            ...
            X_k_1 = paddle.reshape(
                x=X_k_1,
                shape=[-1, s, self.sparse_feature_dim])  # None, s, embedding_size
            #X_k_1 = m(X_k_1)
            Xs.append(X_k_1)
            last_s = s

        # sum pooling
        y_cin = paddle.concat(x=Xs[1:], axis=1)  # None, (num_field++), embedding_size
