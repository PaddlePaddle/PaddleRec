    ...
    for epoch_id in range(last_epoch_id + 1, epochs):
        ...
        if reader_type == 'DataLoader':
            fetch_batch_var, step_num = dataloader_train(
                ...
                config, use_visual, log_visual, step_num,correct,total)

            # ******************
            acc_epoch_res = float(sum(correct)) / sum(total)
            fetch_batch_var[1]=f"{sum(correct)}-{sum(total)}-{acc_epoch_res}"
            # ******************
...
def dataloader_train(epoch_id, train_dataloader, input_data_names, fetch_vars,
                     exe, config, use_visual, log_visual, step_num,correct=None,total=None):
    print_interval = config.get("runner.print_interval", None)
    batch_size = config.get("runner.train_batch_size", None)
    ...
    for batch_id, batch_data in enumerate(train_dataloader()):
        ...
        # ******************
        correct.append(int(batch_size*fetch_batch_var[1]))
        total.append(batch_size)
        # ******************
