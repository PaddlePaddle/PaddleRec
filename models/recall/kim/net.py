...
class AttentivePooling(nn.Layer):
    def __init__(self, dim1=10, dim2=100, dropout=0.2):
        ...
        self.dense1 = nn.Sequential(nn.Linear(dim2, dim1), nn.Tanh())
        self.dense2 = nn.Sequential(nn.Linear(dim1, 1))
...
class PairPair(nn.Layer):
    ...
    def forward(self, entity_input):
        ...
        user_entity_zerohop = user_entity_input[..., max_entity_num *
                                                max_entity_num:, :]  # bz,1,100
        user_entity_onehop = user_entity_input[..., :max_entity_num *
                                               max_entity_num, :]
        user_entity_onehop = user_entity_onehop.reshape(
            (-1, max_entity_num, max_entity_num, 100))
        ...
        user_can = self.gat_fuse(
            user_can.astype('float32'))  # (max_entity_num,100)
        ...
        news_entity_zerohop = news_entity_input[..., max_entity_num *
                                                max_entity_num:, :]  # bz,1,100
        news_entity_onehop = news_entity_input[..., :max_entity_num *
                                               max_entity_num, :]
        ...
        news_can = self.gat_fuse(
            news_can.astype('float32'))  # (max_entity_num,100)
        ...
        user_entity_onehop = paddle.concat(
            ...
            -2)  # bz,max_entity_num,max_entity_num*2,100
...
class PairModel(nn.Layer):
    ...
    def forward(self, title_inputs, entity_inputs, one_hop_inputs,
        ...
        clicked_title_att_vecs = clicked_title_att_vecs.reshape(
            [-1, MAX_SENTS * MAX_SENTENCE, 200])  # (bz,50*30,200)
        ...
        news_entity = news_entity.reshape((
            -1,
            max_entity_num * (max_entity_num + 1) * 100, ))

        news_entity = news_entity.unsqueeze(1).tile([1, MAX_SENTS, 1])
        news_entity = news_entity.reshape(
            (-1, MAX_SENTS, max_entity_num * (max_entity_num + 1), 100))
