...
def main(args):
    ...
    for epoch_id in range(start_epoch, end_epoch):
        ...
        for batch_id, batch_data in enumerate(test_dataloader()):
            ...
            # print(np.array(batch_data[1]))
            # continue
            # print(np.array(batch_data[2]))
            # print(input_data_names)
            # break
            user_embs = fetch_batch_var[0]
            # faiss.normalize_L2(user_embs)
            # print(np.array(user_embs)[0])
        ...
        ndgc = total_ndcg / total
        hitrate = total_hitrate * 1.0 / total
        metric_str = ""
        metric_str += "recall@{}:{} ".format(args.top_n, recall)
        metric_str += "ndgc@{}:{} ".format(args.top_n, ndgc)
        metric_str += "hitrate@{}:{} ".format(args.top_n, hitrate)
