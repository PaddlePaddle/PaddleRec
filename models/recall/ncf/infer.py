...
def main(args):
    ...
    for epoch_id in range(start_epoch, end_epoch):
        ...
        for batch_id, batch in enumerate(test_dataloader()):
            ...
            predictions = paddle.squeeze(tensor_print_dict['prediction'])
            _, indices = paddle.topk(predictions, 10)
            indices = indices.numpy().tolist()

            if 0 in indices:
                rank = indices.index(0)
                NDCG += 1 / np.log2(rank + 2)
                HT += 1
        NDCG = NDCG / valid_user
        HT = HT / valid_user
        metric_str = "NDCG@10 : {:.6f},".format(
            NDCG) + " HR@10 : {:.6f},".format(HT)
