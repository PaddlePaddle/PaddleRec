...
def infer_network(vocab_size, emb_size, pure_bf16=False):
    ...
    emb_all_label = fluid.embedding(input=all_label,
                                    size=[vocab_size, emb_size],
                                    param_attr="emb",
                                    dtype=dtype)

    emb_a = fluid.embedding(input=analogy_a,
                            size=[vocab_size, emb_size],
                            param_attr="emb",
                            dtype=dtype)
    emb_b = fluid.embedding(input=analogy_b,
                            size=[vocab_size, emb_size],
                            param_attr="emb",
                            dtype=dtype)
    emb_c = fluid.embedding(input=analogy_c,
                            size=[vocab_size, emb_size],
                            param_attr="emb",
                            dtype=dtype)
...
def reader_creator(file_dir, word_to_id):
    def reader():
        ...
        for fi in files:
            with io.open(os.path.join(file_dir, fi), "r",
                         encoding='utf-8') as f:
                for line in f:
                    if ':' in line:
                        pass
...
if __name__ == "__main__":
    ...
    vocab_size, test_reader, id2word = prepare_data(test_dir,
                                                    dict_path,
