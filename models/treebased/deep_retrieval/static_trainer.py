...
def main(args):
    ...
    with paddle.static.program_guard(em_main_program, em_startup_program):
        static_user_emb =             paddle.static.data(
                ...
                dtype='float32')       
        static_saved_path, static_final_prob = static_model_class.model.generate_candidate_path_for_item(static_user_emb, beam_size)
        ...
        if record_item_to_user_emb:
            ...
            for key in item_to_user_emb:
                ...
                if not pernalize_path_to_item_count:
                    ...
                    for single_path,single_pro in zip(path,pro):
                        list.append(str(static_model_class.graph_index.kd_represent_to_path_id(single_path)) + ":" + str(single_pro))
