...
class Model(ModelBase):
    ...
    def train_input(self):
        input_emb = paddle.static.data(
            name="input_emb",
            shape=[None, self.input_emb_size],
            dtype="float32", )

        item_label = paddle.static.data(
            name="item_label",
            shape=[None, 1],
            dtype="int64", )
    ...
    def tdm_net(self, input):
        ...
        if self.check_version():
            with paddle.fluid.device_guard("cpu"):
                sample_nodes, sample_label, sample_mask = paddle.fluid.contrib.layers.tdm_sampler(
                    x=item_label,
                    neg_samples_num_list=self.neg_sampling_list,
                    layer_node_num_list=self.layer_node_num_list,
                    leaf_node_num=self.leaf_node_nums,
                    tree_travel_attr=paddle.ParamAttr(name="TDM_Tree_Travel"),
                    tree_layer_attr=paddle.ParamAttr(name="TDM_Tree_Layer"),
        ...
        sample_nodes = [
            paddle.fluid.layers.nn.reshape(sample_nodes[i], [-1, 1])
        ...
        sample_nodes_emb = [
            paddle.static.nn.embedding(
                ...
                param_attr=paddle.ParamAttr(name="TDM_Tree_Emb"))
            for i in range(self.max_layers)
        ]

        # 此处进行Reshape是为了之后层次化的分类器训练
        sample_nodes_emb = [
            paddle.fluid.layers.nn.reshape(sample_nodes_emb[i], [
        ...
        tdm_fc = paddle.static.nn.fc(
            x=layer_classifier_res,
            size=2,
            activation=None,
            num_flatten_dims=2,
            weight_attr=paddle.ParamAttr(name="tdm.cls_fc.weight"),
            bias_attr=paddle.ParamAttr(name="tdm.cls_fc.bias"))

        # 将loss打平，放到一起计算整体网络的loss
        tdm_fc_re = paddle.fluid.layers.nn.reshape(tdm_fc, [-1, 2])

        # 若想对各个层次的loss辅以不同的权重，则在此处无需concat
        # 支持各个层次分别计算loss，再乘相应的权重
        sample_label = paddle.concat(x=sample_label, axis=1)
        labels_reshape = paddle.fluid.layers.nn.reshape(sample_label, [-1, 1])
        ...
        # 通过mask过滤掉虚拟节点的loss
        sample_mask = paddle.concat(x=sample_mask, axis=1)
        mask_reshape = paddle.fluid.layers.nn.reshape(sample_mask, [-1, 1])
        mask_index = paddle.fluid.layers.where(mask_reshape != 0)
        mask_index.stop_gradient = True

        self.mask_cost = paddle.gather_nd(x=cost, index=mask_index)

        softmax_prob = paddle.unsqueeze(a=softmax_prob, axis=[1])
        self.mask_prob = paddle.gather_nd(x=softmax_prob, index=mask_index)
        self.mask_label = paddle.gather_nd(x=labels_reshape, index=mask_index)

        self._predict = self.mask_prob

    def create_info(self):
        paddle.static.default_startup_program().global_block().create_var(
            name="TDM_Tree_Info",
            dtype=paddle.fluid.core.VarDesc.VarType.INT32,
            shape=[self.node_nums, 3 + self.child_nums],
            persistable=True,
            initializer=paddle.nn.initializer.Constant(value=0))
        paddle.static.default_main_program().global_block().create_var(
            name="TDM_Tree_Info",
            dtype=paddle.fluid.core.VarDesc.VarType.INT32,
            shape=[self.node_nums, 3 + self.child_nums],
            persistable=True)
    ...
    def avg_loss(self):
        avg_cost = paddle.mean(x=self.mask_cost)
        self._cost = avg_cost

    def metrics(self):
        auc, batch_auc, _ = paddle.fluid.layers.auc(input=self._predict,
    ...
    def input_trans_layer(self, input_emb):
        ...
        """
        # 将input映射到与node相同的维度
        input_fc_out = paddle.static.nn.fc(
            x=input_emb,
            size=self.node_emb_size,
            activation=None,
            weight_attr=paddle.ParamAttr(name="trans.input_fc.weight"),
            bias_attr=paddle.ParamAttr(name="trans.input_fc.bias"), )
    ...
    def _expand_layer(self, input_layer, node, layer_idx):
        ...
        # 同时兼容了训练组网与预测组网
        input_layer_unsequeeze = paddle.unsqueeze(a=input_layer, axis=[1])
        if not isinstance(node, list):
            input_layer_expand = paddle.fluid.layers.expand(
    ...
    def classifier_layer(self, input, node):
        ...
        input_node_concat = [
            paddle.concat(
                x=[input_expand[i], node[i]], axis=2)
            for i in range(self.max_layers)
        ]
        hidden_states_fc = [
            paddle.static.nn.fc(x=input_node_concat[i],
                                size=self.node_emb_size,
                                num_flatten_dims=2,
                                activation=self.act,
                                weight_attr=paddle.ParamAttr(
        ...
        hidden_states_concat = paddle.concat(x=hidden_states_fc, axis=1)
    ...
    def create_first_layer(self):
        ...
        for id in first_layer_node:
            node_list.append(
                paddle.full(
                    shape=[self.batch_size, 1], fill_value=int(id)))
            mask_list.append(
                paddle.full(
                    shape=[self.batch_size, 1], fill_value=0))
        self.first_layer_node = paddle.concat(x=node_list, axis=1)
        self.first_layer_node_mask = paddle.concat(x=mask_list, axis=1)
    ...
    def tdm_infer_net(self, input):
        ...
        for layer_idx in range(self.first_layer_idx, self.max_layers):
            ...
            current_layer_node = paddle.fluid.layers.nn.reshape(
                current_layer_node, [-1, current_layer_node_num])
            current_layer_node_mask = paddle.fluid.layers.nn.reshape(
                current_layer_node_mask, [-1, current_layer_node_num])
            node_emb = paddle.static.nn.embedding(
            ...
            prob = paddle.nn.functional.softmax(x=tdm_fc)
            positive_prob = paddle.slice(prob, axes=[2], starts=[1], ends=[2])
            prob_re = paddle.fluid.layers.nn.reshape(
                positive_prob, [-1, current_layer_node_num])

            # 过滤掉padding产生的无效节点（node_id=0）
            node_zero_mask = paddle.cast(current_layer_node, 'bool')
            node_zero_mask = paddle.cast(node_zero_mask, 'float32')
            ...
            if layer_idx < self.max_layers - 1:
                ...
                current_layer_node, current_layer_node_mask = \
                    paddle.fluid.contrib.layers.tdm_child(x=top_node,
        ...
        total_node_score = paddle.concat(x=node_score, axis=1)
        total_node = paddle.concat(x=node_list, axis=1)

        # 考虑到树可能是不平衡的，计算所有层的叶子节点的topK
        res_score, res_i = paddle.topk(x=total_node_score, k=self.topK)
        res_layer_node = paddle.fluid.contrib.layers.index_sample(total_node,
                                                                  res_i)
        res_node = paddle.fluid.layers.nn.reshape(res_layer_node,
                                                  [-1, self.topK, 1])

        # 利用Tree_info信息，将node_id转换为item_id
        tree_info = paddle.static.default_main_program().global_block().var(
            "TDM_Tree_Info")
        res_node_emb = paddle.gather_nd(x=tree_info, index=res_node)

        res_item = paddle.slice(res_node_emb, axes=[2], starts=[0], ends=[1])
        self.res_item_re = paddle.fluid.layers.nn.reshape(res_item,
                                                          [-1, self.topK])
    ...
    def layer_fc_infer(self, input_fc_out, layer_idx):
        ...
        input_layer_fc_out = paddle.static.nn.fc(
            x=input_fc_out,
            size=self.node_emb_size,
            activation=self.act,
            weight_attr=paddle.ParamAttr(
    ...
    def classifier_layer_infer(self, input, node, layer_idx):
        ...
        # 与训练网络相同的concat逻辑
        input_node_concat = paddle.concat(x=[input_expand, node], axis=2)

        # 根据参数名param_attr调用不同的层的FC
        hidden_states_fc = paddle.static.nn.fc(
            x=input_node_concat,
            size=self.node_emb_size,
            num_flatten_dims=2,
            activation=self.act,
    ...
    def check_version(self):
        ...
        try:
            paddle.utils.require_version('1.8.0')
            return True
