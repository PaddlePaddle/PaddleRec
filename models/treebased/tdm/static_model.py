...
class StaticModel():
    ...
    def net(self, inputs, is_infer=False):
        ...
        vocab_embeddings = np.zeros((self.node_nums, self.node_emb_size))
        for ind, word in enumerate(word2emb.keys()):
            emb = word2emb.get(word, np.zeros((self.node_emb_size,)))
            vocab_embeddings[int(word), :] = emb
        '''
        ...
        cost, softmax_prob = paddle.nn.functional.softmax_with_cross_entropy(
            ...
            return_softmax=True,
            ignore_index=-1)
        #paddle.static.Print(inputs[-2], summarize=-1)
