...
class Model(ModelBase):
    ...
    def tdm_att_net(self, input):
        ...
        user_node_concat = self.attention_interactive_layer(
            user_feature_emb, sample_nodes_emb, user_feature_mask)
    ...
    def attention_interactive_layer(self, user_feature_emb, sample_nodes_emb,
                                    user_feature_mask):
        ...
        user_node_concat = [
            fluid.layers.concat(
                [
                    user_feature_emb_expand[i], user_node_product[i],
                    sample_nodes_emb
                ],
                axis=2) for i in range(self.item_nums)
        ]
        ...
        att_fc1 = [
            fluid.layers.fc(
                ...
                act="relu",
                param_attr=fluid.ParamAttr(
                    name="att.fc_1.weight",
                    initializer=fluid.initializer.Normal(
                        scale=1.0 / math.sqrt(self.node_emb_size * 3))),
                bias_attr=fluid.ParamAttr(
                    name="att.fc_1.bias",
                    initializer=fluid.initializer.Constant(0.1)))
            for i in range(self.item_nums)
        ...
        att_fc2 = [
            fluid.layers.fc(att_fc1[i],
                            size=1,
                            num_flatten_dims=2,
                            act="relu",
                            param_attr=fluid.ParamAttr(
                                name="att.fc_2.weight",
                                initializer=fluid.initializer.Normal(
                                    scale=1.0 / math.sqrt(36))),
                            bias_attr=fluid.ParamAttr(
                                name="att.fc_2.bias",
                                initializer=fluid.initializer.Constant(0.1)))
            for i in range(self.item_nums)
        ...
        att_mask = [
            fluid.layers.elementwise_mul(att_res[i],
        ...
        for feasign_grout_length in self.fea_group:
            ...
            current_fea_group = att_mask[start_item_index:end_item_index]
