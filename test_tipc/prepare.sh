...
elif [ ${model_name} == "dcn_v2" ]; then
    mkdir -p ./test_tipc/data/train
    mkdir -p ./test_tipc/data/infer
    if [ ${MODE} = "lite_train_lite_infer" ];then
        cp -r ./models/rank/dcn_v2/data/sample_data/train/* ./test_tipc/data/train
        cp -r ./models/rank/dcn_v2/data/sample_data/train/* ./test_tipc/data/infer
        echo "demo data ready"
    elif [ ${MODE} = "whole_train_whole_infer" ];then
        cd ./datasets/criteo_dcn_v2
        bash run.sh
        cd ../..
        cp -r ./datasets/criteo_dcn_v2/slot_train_data_full/* ./test_tipc/data/train
        cp -r ./datasets/criteo_dcn_v2/slot_test_data_full/* ./test_tipc/data/infer
        echo "whole data ready"
    elif [ ${MODE} = "whole_infer" ];then
        cd ./datasets/criteo_dcn_v2
        bash run.sh
