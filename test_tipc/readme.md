...
2. 基础训练预测配置中需要用到动转静部分，由于不同模型的动转静脚本不同，放置于模型名称的子目录下面，目前统一命名为`to_static.py`
...
- [test_train_inference_python 使用](./doc/test_train_inference_python.md) ：测试基于Python的模型训练、评估、推理等基本功能。
