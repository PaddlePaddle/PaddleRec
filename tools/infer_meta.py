...
def main(args):
    ...
    train_dataloader = create_data_loader(config=config, place=place, mode="test")
    ...
    for epoch_id in range(start_epoch, end_epoch):
        ...
        for batch_id, batch in enumerate(train_dataloader()):
            ...
            # 对每个子任务进行训练
            optimizer = dy_model_class.create_optimizer(dy_model, config, "infer")
            dy_model.train()
            ...
            for i in range(infer_train_epoch):
                ...
                for i_batch in range(n_batch):
                    batch_input = list()
                    batch_x = []
                    batch_x.append(x_spt[0][i_batch * batchsize: (i_batch + 1) * batchsize])
                    batch_x.append(x_spt[1][i_batch * batchsize: (i_batch + 1) * batchsize])
                    batch_x.append(x_spt[2][i_batch * batchsize: (i_batch + 1) * batchsize])
                    batch_x.append(x_spt[3][i_batch * batchsize: (i_batch + 1) * batchsize])

                    batch_y = y_spt[i_batch * batchsize: (i_batch + 1) * batchsize]
            ...
            with paddle.no_grad():
                ...
                for i_batch in range(n_batch):
                    batch_input = list()
                    batch_x = []
                    batch_x.append(x_qry[0][i_batch * batchsize: (i_batch + 1) * batchsize])
                    batch_x.append(x_qry[1][i_batch * batchsize: (i_batch + 1) * batchsize])
                    batch_x.append(x_qry[2][i_batch * batchsize: (i_batch + 1) * batchsize])
                    batch_x.append(x_qry[3][i_batch * batchsize: (i_batch + 1) * batchsize])

                    batch_y = y_qry[i_batch * batchsize: (i_batch + 1) * batchsize]

                    batch_input.append(batch_x)
                    batch_input.append(batch_y)

                    metric_list, metric_list_local = dy_model_class.infer_forward(
                        dy_model, metric_list, metric_list_local, batch_input, config)
            ...
            for metric_id in range(len(metric_list_local_name)):
                metric_str_local += (
                    metric_list_local_name[metric_id] +
                    ": {:.6f},".format(metric_list_local[metric_id].accumulate())
                )
                if use_visual:
            ...
            logger.info(
                "epoch: {}, batch_id: {}, aid: {} "
                .format(epoch_id, batch_id, aid_flag) + metric_str_local + 
                " avg_reader_cost: {:.5f} sec, avg_batch_cost: {:.5f} sec, avg_samples: {:.5f}, ips: {:.2f} ins/s".
                format(infer_reader_cost / print_interval, 
                (infer_reader_cost + infer_run_cost) / print_interval, batchsize, 
                print_interval * batchsize / (time.time() - interval_begin)))
